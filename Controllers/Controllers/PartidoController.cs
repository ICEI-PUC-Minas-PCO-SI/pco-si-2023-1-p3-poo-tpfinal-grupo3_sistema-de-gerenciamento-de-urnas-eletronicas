    ...
    public class PartidoController : ControllerBase<Partido>
    {
        ...
        public void InsertUpdate(List<PartidoDTO> data)
        {
            using (var cnx = new UrnaEletronicaContext())
            {
                ...
                var partidosRepetidos = dbSet.AsNoTracking().AsEnumerable().Where(x => dataModelInsert.Any(y => y.Numero == x.Numero)).ToList();
                ...
                partidosRepetidos = dbSet.AsNoTracking().AsEnumerable().Where(x => dataModelUpdate.Any(y => y.Id != x.Id && y.Numero == x.Numero)).ToList();
